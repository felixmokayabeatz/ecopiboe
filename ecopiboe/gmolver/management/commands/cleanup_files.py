...
class Command(BaseCommand):
    help = 'Delete files from uploads folder older than twelve hours'
    ...
    def handle(self, *args, **kwargs):
        ...
        # Find files older than 12 hours
        ...
        if total_files > 0:
            # Print the total number of files to be deleted
            ...
            if confirmation == 'YES':
                for i, file_path in enumerate(files_to_delete):
                    ...
                    sys.stdout.flush()
                    time.sleep(0.001) 
                self.stdout.write('\nAll files Older than 4 hours have been deleted.')
            else:
                self.stdout.write(self.style.NOTICE('Operation cancelled. No files were deleted. (YES/no ==> CASE SENSITIVE)'))
        else:
            self.stdout.write(self.style.NOTICE('No files older than 12 hours found.'))
